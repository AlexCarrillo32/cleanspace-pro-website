# CleanSpace Pro - Development Guidelines

## Implementation Best Practices

### 0 — Purpose

These rules ensure maintainability, safety, and developer velocity for the CleanSpace Pro cleaning service website and backend API.
...
- **D-1 (MUST)** Type DB helpers to work with both connection instances and transaction objects when using SQLite3.
- **D-2 (SHOULD)** Use prepared statements for all user input to prevent SQL injection.
...
- **O-2 (MUST)** Separate frontend logic (HTML/CSS/JS) from backend API logic.
...
6. Is the function easily testable without mocking core features (e.g. sql queries, express requests, etc.)? If not, can this function be tested as part of an integration test?
...
- Frontend: `index.html`, `styles.css`, `script.js` - Static website files
- Backend: `server.js` - Express.js API server
- Source: `src/` - Shared utilities and backend logic
- Configuration: `package.json`, `.env.example` - Project setup and environment
